...
public class Solution {
    ...
    */
    ...
    public static void main(String[] args) {
        Task task1 = new Task(1, "Read Version Control with Git book", TaskType.READING, LocalDate.of(2015, Month.JULY, 5)).addTag("git").addTag("reading").addTag("books");
    ...
    private static List<Task> allReadingTasks(List<Task> tasks) {
        // Ваш код здесь
        return tasks.stream()
                .filter(x -> x.getType() == TaskType.READING)
                .distinct()
                .toList();
    }
}